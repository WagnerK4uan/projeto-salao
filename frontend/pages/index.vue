...
<template>
  ...
  <div class="min-h-screen flex flex-col items-center justify-start pt-8 " >
    <h1 class="text-3xl font-bold mb-4 text-gray-700">Clientes Cadastradas</h1>
    <input
      v-model="busca"
      type="text"
      placeholder="Pesquisar por nome..."
      class="mb-6 px-4 py-2 border rounded w-full max-w-md bg-gray-50 text-gray-700"
    />
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      >
        <h2 class="text-xl font-semibold text-gray-800">{{ cliente.nome }}</h2>
        <p class="text-sm text-gray-600">CPF: {{ cliente.cpf }}</p>
        <p class="text-sm text-gray-600">Data de Nascimento: {{ cliente.data_nascimento }}</p>
        <div class="mt-2">
